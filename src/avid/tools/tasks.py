...
class TaskStore:
    ...
    def __init__(self, directory: Path = TASKS_DIR) -> None:
        ...
        # 可重入：外壳把"读 → 判断 → 写"整段括起来，里面的 load/save 再取一次不会自锁。
...
def incomplete_dependencies(task: Task | None) -> list[str]:
    ...
    for dependency in task.blockedBy:
        upstream = TASKS.load(dependency)
...
def create_task_tool(args: dict[str, Any]) -> str:
    try:
        subject = _string(args, "subject", "")
        description = _string(args, "description", "")
        task = create_task(subject or "", description or "")
    except TaskError as exc:
        return f"错误：{exc}"
    return task.id


def update_task_tool(args: dict[str, Any]) -> str:
    try:
        task_id = _string(args, "task_id", "")
        task = update_task(task_id or "", _string_list(args, "addBlockedBy"))
    except TaskError as exc:
        return f"错误：{exc}"
    return json.dumps(asdict(task), ensure_ascii=False, indent=2)


def can_start_tool(args: dict[str, Any]) -> str:
    try:
        task_id = _string(args, "task_id", "")
        return str(can_start(task_id or ""))
    except TaskError as exc:
        return f"错误：{exc}"


def _transition(args: dict[str, Any], action) -> str:
    ...
    with TASKS.lock:
        try:
            if TASKS.load(task_id or "") is None:
                return f"错误：找不到任务 {task_id}"
        except TaskError as exc:
            return f"错误：{exc}"
        return action(task_id or "", owner or "agent")


def claim_task_tool(args: dict[str, Any]) -> str:
    return _transition(args, claim_task)


def complete_task_tool(args: dict[str, Any]) -> str:
    return _transition(args, complete_task)


def get_task_tool(args: dict[str, Any]) -> str:
    try:
        task_id = _string(args, "task_id", "")
        if TASKS.load(task_id or "") is None:
            return f"错误：找不到任务 {task_id}"
