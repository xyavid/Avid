...
- **验收基准**：参考场景 **R**（读取本地文件 + 计算）——它是阶段 1 首个工具的来源，也是阶段 4 评测集的来源。
- **技术栈**：内核 Python 3.12，环境与依赖管理用 `uv`；TypeScript 前端留到阶段 7。
- **当前状态**：需求与阶段路线已成文，尚在阶段 0。

## 2. 提交规范

自阶段 0 的 `git init` 起，此后每完成一个可验证的小步就提交一次；同一阶段内不攒成一个大提交。
...
"阶段"指 `docs/roadmap.md` 中的一章。**完整阶段**动工前先出图；阶段内部的细碎修改不走此流程。

### 第一步：出图（动工前，一条消息给全）

1. **文件清单**：本阶段新增 / 修改 / 删除的每个文件，各附一句话职责。
2. **目录树**：本阶段完成后的完整目录结构，用文本树展示。
3. **架构图**：本阶段完成后的模块划分与数据流向，手写 SVG 落盘到 `docs/architecture/phase-<阶段号>-<短名>.svg`（无外部依赖，浏览器可直接打开），图中标注每条数据流携带的内容，随本阶段首次提交入库。
...
代码完成后，按 `docs/roadmap.md` 中该阶段的**验收标准逐条**给出证据：可执行的命令加上实际输出。然后勾选该文件末尾的"当前进度"。

### 结束条件

该阶段每条验收标准都有证据、进度已勾选、且用户确认后，才进入下一阶段。
