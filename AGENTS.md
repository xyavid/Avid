...
- **当前状态**：阶段 0（工程基线与最小模型调用）已完成，阶段 1 未开始。路线进度在 `dev/plan/roadmap.md`。
...
"阶段"指 `dev/plan/roadmap.md` 中的一章。**完整阶段**动工前先出图；阶段内部的细碎修改不走此流程。
...
代码完成后，按 `dev/plan/roadmap.md` 中该阶段的**验收标准逐条**给出证据：可执行的命令加上实际输出。然后勾选该文件末尾的"当前进度"。
