...
- **当前状态**：最小模型调用、Agent 循环、8 个工具（`bash` / `read_file` / `write_file` / `edit_file` / `glob` / `todo_write` / `subagent` / `load_skill`）、技能系统、上下文压缩管线、权限三闸门与 hook 扩展点、会话持久化（`session/`）均已跑通。项目目标见 `dev/plan/roadmap.md`。
